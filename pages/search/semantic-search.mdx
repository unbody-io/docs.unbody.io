...
## Basic Search
Let's say you want to find documents discussing "AI applications" - but the relevant content might use terms like "machine learning systems" or "neural networks" instead. Semantic search []() understands these conceptual relationships and returns relevant results regardless of exact term matches.
...
Let's say you need to perform a semantic search for "AI applications" but want to prioritize results that are more closely related to specific concepts like "Deep Learning," "LLM," and "chatbot." To achieve this, we can use concept boosting, which adjusts the search results to align more strongly with these concepts by applying a weighted influence (force factor). This ensures that the most relevant results, based on the given priorities, are returned.
...
When building an AI knowledge base, you might want to find modern AI-related content while specifically excluding outdated approaches. For example, when searching for chatbot documentation, you want to focus on modern NLP and Large Language Models while avoiding legacy rule-based systems. Concept steering lets you push the search toward desired concepts and away from unwanted ones simultaneously.
