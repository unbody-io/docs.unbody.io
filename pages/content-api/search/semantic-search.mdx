...
This guide is designed for developers who have a good grasp of the [basics of semantic search](/graphql-api/search/semantic-search) and are ready to explore more sophisticated functionalities to enhance their search capabilities. Let’s dive in, folks!
...
<Tabs size={"sm"} variant={"underlined"} className={"mt-2"}>
    <Tab title={"Unbody SDK"}>
        ```javascript copy
        await unbody.get.textDocument.search.find("web development", ["title"]).exec()
        ```
    </Tab>
    <Tab title={"GraphQl formatted"}>
        ```graphql
