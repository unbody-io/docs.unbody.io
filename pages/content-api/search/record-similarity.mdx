...
<Tabs size={"sm"} variant={"underlined"} className={"mt-2"}>
    <Tab title={"Unbody SDK"}>
        ```javascript copy
        await unbody.get.<object_name>.similar.record(objectId, {...options}).exec();
        ```
    </Tab>
    <Tab title={"GraphQL"}>
...
<Tabs size={"sm"} variant={"underlined"} className={"mt-2"}>
    <Tab title={"Unbody SDK"}>
        ```javascript copy
        await unbody.get.googleDoc.similar.record('specificDocId').exec()
        ```
    </Tab>
    <Tab title={"GraphQL"}>
...
<Tabs size={"sm"} variant={"underlined"} className={"mt-2"}>
    <Tab title={"Unbody SDK"}>
        ```javascript copy
        await unbody.get.discordMessage.similar.record("specificMessageId").exec()
...
<Tabs size={"sm"} variant={"underlined"} className={"mt-2"}>
    <Tab title={"Unbody SDK"}>
        ```javascript copy
        await unbody.get.textBlock.similar.record("specificTextBlockId").exec();
