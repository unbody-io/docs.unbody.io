...
<Tabs size={"sm"} variant={"underlined"} className={"mt-2"}>
    <Tab title={"Unbody SDK"}>
        ...
        const {data: {payload}} = await unbody.get
            .googleDoc
            .search
            .hybrid("specific keyword and broad concept", {properties: ["title^2", "text"]})
            .exec();
        ```
    </Tab>
    <Tab title={"GraphQl"}>
